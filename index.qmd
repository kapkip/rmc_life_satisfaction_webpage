...
filter_cantril <- cantril %>% 
  ...
  mutate(
    ...
    `Relative change` = round((`2023` - `2011`) / `2011`, 2)
  )

reactable(filter_cantril,
...
# Create line chart
cantril_round <- cantril %>% 
  mutate(`Cantril ladder score` = round(`Cantril ladder score`, 2))

cantril_plot <- ggplot(cantril_round %>% 
  filter(Country %in% c("China", "India", "United States", "Indonesia", "Pakistan", "Nigeria"))) +
    geom_line(aes(x = `Year`, y = `Cantril ladder score`, color = `Country`))+
  scale_y_continuous(limits = c(0,10)) +
  labs(y = "Cantril ladder score") +
  theme_minimal() 
