...
filter_cantril <- cantril %>% 
  ...
  mutate(`Relative change` = round((`2023`-`2011`) / `2011`, 2)) #divide by original value
...
# Create line chart
cantril_plot <- ggplot(cantril %>% 
  filter(Country %in% c("China", "India", "United States", "Indonesia"))) +
